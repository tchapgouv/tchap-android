    ...
    private fun renderSummaryText(holder: Holder, data: Data?) {
        ...
        val summary = if (createdFromCurrentUser) {
            ...
            if (isDirectRoom) {
                ...
                resources.getString(CommonStrings.direct_room_created_summary_item, TchapUtils.getNameFromDisplayName(data?.memberName.orEmpty()))
            } else {
        ...
        val description = when {
            isDirectRoom -> {
                if (attributes.isLocalRoom) {
                    resources.getString(CommonStrings.send_your_first_msg_to_invite, roomDisplayName)
                } else {
                    resources.getString(CommonStrings.this_is_the_beginning_of_dm, roomDisplayName)
                }
            }
