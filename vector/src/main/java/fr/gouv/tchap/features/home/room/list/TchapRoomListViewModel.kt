...
import im.vector.app.features.home.room.list.RoomCategory
...
import im.vector.app.features.home.room.list.RoomSummaries
...
class TchapRoomListViewModel @Inject constructor(initialState: RoomListViewState,
                                                 private val session: Session,
                                                 private val roomSummariesSource: DataSource<List<RoomSummary>>)
    : VectorViewModel<RoomListViewState, RoomListAction, RoomListViewEvents>(initialState) {

    interface Factory {
        fun create(initialState: RoomListViewState): TchapRoomListViewModel
    }

    companion object : MvRxViewModelFactory<TchapRoomListViewModel, RoomListViewState> {

        @JvmStatic
        override fun create(viewModelContext: ViewModelContext, state: RoomListViewState): TchapRoomListViewModel? {
            val fragment: TchapRoomListFragment = (viewModelContext as FragmentViewModelContext).fragment()
            return fragment.roomListViewModelFactory.create(state)
        }
    }

    private val displayMode = initialState.displayMode
    private val roomListDisplayModeFilter = TchapRoomListDisplayModeFilter(displayMode)
