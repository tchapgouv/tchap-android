...
class TchapHomeActivity :
    ...
    private val homeActivityViewModel: TchapHomeActivityViewModel by viewModel()
    @Inject lateinit var viewModelFactory: TchapHomeActivityViewModel.Factory
                ...
                .subscribe { sharedAction ->
                    when (sharedAction) {
                        ...
                        is HomeActivitySharedAction.OpenGroup   -> {
                            ...
                            replaceFragment(R.id.homeDetailFragmentContainer, TchapHomeDetailFragment::class.java, allowStateLoss = true)
                        }
                    }.exhaustive
                }
                .disposeOnDestroy()
    ...
    private fun handlePromptToEnablePush() {
        popupAlertManager.postVectorAlert(
                DefaultVectorAlert(
                        ...
                        shouldBeDisplayedIn = {
                            it is TchapHomeActivity
                        }
                ).apply {
                    colorInt = ThemeUtils.getColor(this@TchapHomeActivity, R.attr.vctr_notice_secondary)
    ...
    private fun promptSecurityEvent(userItem: MatrixItem.UserItem?, titleRes: Int, descRes: Int, action: ((VectorBaseActivity<*>) -> Unit)) {
        popupAlertManager.postVectorAlert(
                VerificationVectorAlert(
                    ...
                    colorInt = ContextCompat.getColor(this@TchapHomeActivity, R.color.riotx_positive_accent)
                    contentAction = Runnable {
                        (weakCurrentActivity?.get() as? VectorBaseActivity<*>)?.let {
                            action(it)
                        }
    ...
    override fun navToMemberProfile(userId: String, deepLink: Uri): Boolean {
        val listener = object : MatrixToBottomSheet.InteractionListener {
            override fun navigateToRoom(roomId: String) {
                navigator.openRoom(this@TchapHomeActivity, roomId)
            }
        }
    ...
    companion object {
        fun newIntent(context: Context, clearNotification: Boolean = false, accountCreation: Boolean = false): Intent {
            ...
            return Intent(context, TchapHomeActivity::class.java)
                    .apply {
